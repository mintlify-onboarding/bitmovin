---
title: "How to setup an Akamai NetStorage Input"
---
...
To create a Bitmovin ["Akamai NetStorage" Input configuration](/encoding/reference/postencodinginputsakamainetstorage), that can be used by our encoding service, you need to know the `HTTP API Key` and `Username` of your upload account, as well as its `Host` URL.

### Use the Dashboard UI

1. Select the `Encoding` menu on the left and go to `Inputs`.
2. Click on `Create` in the upper right corner of the Inputs overview
3. Select `Akamai` as Input type and
4. Enter all required fields (please see the image below)
5. Click on `Create`

![](/images/encoding/docs/b11c9aa-image.png)
